...
//            sesion.setDebug(true);
            ...
            errorBitacora(1, desc, "ERROR");
            logger.error("conectPOP3 - ERROR " + e);
            logger.error("conectPOP3 - ERROR " + e.getMessage());
            logger.error("conectPOP3 - ERROR " + e.getCause());
            logger.error("conectPOP3 - ERROR " + e.toString());
            e.printStackTrace();
        }
        return con;
    }
    ...
    public static boolean conIMAP(String servidor, String puerto, String usuario, String pass, String tls, String ssl) {
        ...
        try {
            ...
            String desc = "conectIMAP - MessagingException ERROR " + e;
            errorBitacora(1, desc, "ERROR");
            logger.error("conectIMAP ERROR " + e);
            logger.error("conectIMAP - ERROR " + e.getMessage());
            logger.error("conectIMAP - ERROR " + e.getCause());
            logger.error("conectIMAP - ERROR " + e.toString());
        }
        return con;
    }

    public static boolean connect(String server, String username, String password, String port, boolean sslConnection)
            throws Exception {
        ...
        pop3Props.setProperty("mail.pop3.port", String.valueOf(port));
        pop3Props.setProperty("mail.pop3.connectiontimeout", String.valueOf(30000));
        pop3Props.setProperty("mail.pop3.timeout", String.valueOf(30000));
        pop3Props.setProperty("mail.pop3.ssl.protocols", "SSLv3");
        ...
        URLName url = new URLName("pop3", server, puerto, "", username, password);
        ...
        session.setDebug(true);
        logger.debug(new StringBuilder().append("Conectando con ").append(server).append(":").append(port).append(" user: ").append(username).toString());
        store.connect();
        return store.isConnected();
    }
    
    public static boolean connectIMAP(String server, String username, String password, String port, boolean sslConnection)
            throws Exception {
        logger.debug("Creando conexion");
        int puerto = Integer.parseInt(port);
        Properties pop3Props = new Properties();
        ...
        if (sslConnection) {
            pop3Props.setProperty("mail.imap.socketFactory.class", "javax.net.ssl.SSLSocketFactory");
            pop3Props.setProperty("mail.imap.socketFactory.fallback", "false");
            pop3Props.setProperty("mail.imap.socketFactory.port", port);
        }
        pop3Props.setProperty("mail.imap.port", String.valueOf(port));
        pop3Props.setProperty("mail.imap.connectiontimeout", String.valueOf(30000));
        pop3Props.setProperty("mail.imap.timeout", String.valueOf(30000));
        pop3Props.setProperty("mail.imap.ssl.protocols", "SSLv3");
        ...
        URLName url = new URLName("imap", server, puerto, "", username, password);
