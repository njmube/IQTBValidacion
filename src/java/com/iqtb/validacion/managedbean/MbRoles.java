...
public class MbRoles implements Serializable {
    ...
    public MbRoles() {
        ...
        System.out.println("Constructor Roles");
    ...
    public void insertRol() {
        ...
        if (dualListOpciones.getTarget().size() <= 0) {
            ...
            for (String item : dualListOpciones.getTarget()) {
                System.out.println("OPCION SELECCIONADA " + item);
                for (Opciones var : allOpciones) {
                    System.out.println("Opciones " + var.getDescripcion());
                    if (item.equals(var.getDescripcion())) {
                        opcionesSeleccionadas.add(var);
                        logger.info("updateRol - Opcion " + var.getDescripcion() + " agredada a Opciones selecionadas");
                    }
                }
            }
        }
...
//                        rol = new Roles();
                    ...
                    logger.warn("insertRol - Por favor, introdusca un valor para Tipo");
                    this.msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error", "Por favor, introdusca un valor para Tipo.");
                }
            } else {
                logger.warn("insertRol - Por favor, introdusca un valor para Nombre");
                this.msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error", "Por favor, introdusca un valor para Nombre.");
            }
        } catch (Exception e) {
        ...
        if (dualListOpciones.getTarget().size() <= 0) {
            ...
            for (String item : dualListOpciones.getTarget()) {
                ...
                for (Opciones var : tipoOpciones) {
                    ...
                    if (item.equals(var.getDescripcion())) {
                        opcionesSeleccionadas.add(var);
                        System.out.println("updateRol - Opcion " + var.getDescripcion() + " agredada a Opciones selecionadas");
                        logger.info("updateRol - Opcion " + var.getDescripcion() + " agredada a Opciones selecionadas");
                        break;
                    }
                }
                    ...
                    if (opcionesSeleccionadas.size() > 0) {
                        ...
                        for (Opciones op : opcionesSeleccionadas) {
                            ...
                            if (daoRolesOpciones.insertRolesOpciones(rolesOpciones)) {
                                ...
                                System.out.println("updateRol - " + rol.getNombre() + " se la agrego la opcion " + op.getDescripcion());
                                logger.info("updateRol - " + rol.getNombre() + " se la agrego la opcion " + op.getDescripcion());
                            } else {
                                this.msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error", "Error al asignar la opcion " + op.getDescripcion() + " con el usuario " + rol.getNombre());
                                logger.error("updateUsuario - Error al asignar la opcion " + op.getDescripcion() + " con el usuario " + rol.getNombre());
...
//                    rol = new Roles();
                ...
                logger.warn("updateRol - Por favor, introdusca un valor para Nombre");
                this.msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error", "Por favor, introdusca un valor para Nombre.");
            }
        } catch (Exception e) {
